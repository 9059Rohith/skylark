...
I keep `Deal Status` (Open/On Hold/Dead/Won) separate from the lettered `Deal Stage` funnel. Terminal Won/Lost signals take precedence across both fields because the broad status can lag a later detailed stage such as Work Order Received. Active pipeline includes Open, On Hold, Lead, Qualified, Proposal, and Negotiation; terminal or unknown rows are excluded with explicit counts. Revenue questions use valid Won deal value as a disclosed bookings proxy because the boards do not contain accounting-recognition or cash-collection data. I interpret “quarter” as a calendar quarter unless `FISCAL_YEAR_START_MONTH` changes that policy. Deal scope prefers `Close Date (A)` and falls back to `Tentative Close Date`, completion metrics use `Data Delivery Date`, and cross-board scope retains all usable work-order evidence. A quality question scoped to a period asks permission to use the full board because rows missing dates cannot honestly be assigned to that period.
...
I selected FastAPI/Python 3.11 for typed async data work, Next.js App Router/TypeScript/Tailwind for the streaming evidence UI, Render Docker for the backend, and Vercel for the frontend. `BACKEND_URL` remains server-only. Secrets are injected by hosting platforms and health reports readiness without secret values. The owner's Render workspace was already at its 25-service cap and Railway's trial was expired, so I deployed the public backend to Vercel's official FastAPI runtime as an operational fallback instead of deleting unrelated services. Vercel currently supports Python 3.12 and newer, so the public fallback runs 3.12; the canonical Dockerfile and Render path remain pinned to the requested Python 3.11.

With more time I would add application authentication and audit identity, Postgres-backed checkpoints for multi-worker operation, an OAuth monday app with per-user authorization, observability/redaction audits, richer schema-drift alerts, confidence-assisted entity resolution, browser end-to-end tests against an ephemeral backend, and an owner-run staging deployment using copied non-sensitive boards. The public UI and API use server-side monday board IDs, a read-only token, and the selected model key. I chose the MIT License for the public assessment repository so evaluators can inspect and run the submission under an explicit, familiar grant.
